...
password assignments, token assignments, bearer tokens, api keys, secret keys, session secrets, session ids,
raw/unsafe/tmp capture path references, local tmp paths, `file://` urls, and traversal paths. binary evidence
such as png frames and mp4 recordings is skipped by this text scan and must still come from the redaction
pipeline or native run evidence process.
...
references must be context7 library ids beginning with `/`. behavior labels outside the required vocabulary
are rejected. each reference must set `appliesToAdapterVersion` to the same value as the top-level
