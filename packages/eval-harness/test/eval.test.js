...
test("real target-tool run artifact audit rejects failure logs that contradict passing outcomes", () => {
  const proof = realToolProof("odoo");
  const existing = new Set([
    "evals/runs/odoo/real-proof/step-trace.json",
    "evals/runs/odoo/real-proof/final-screen.png",
  ...
  assert.equal(audit.passed, false);
  assert.equal(audit.findings.some((finding) => finding.message.includes("must state no failure observed")), true);
  assert.equal(audit.findings.some((finding) => finding.message.includes("passed is false")), true);
  assert.equal(audit.findings.some((finding) => finding.message.includes("zero human help is false")), true);
      ...
      if (path.endsWith("manifest.json")) {
        return JSON.stringify({
          ...
          inputEvidence: [
            {
              stepId: "step-001",
              inputEvents: [{ kind: "api", event: "write", apiEndpoint: "/odoo/opportunity" }]
            },
            {
              stepId: "step-002",
...
test("real target-tool run artifact audit rejects non-PNG redacted frame evidence", () => {
  const proof = realToolProof("odoo");
  const existing = new Set([
    "evals/runs/odoo/real-proof/step-trace.json",
    "evals/runs/odoo/real-proof/final-screen.png",
    ...
    "flows/odoo/qualify-opportunity.flow.md",
    "captures/normalized/capture-real-odoo-001/manifest.json",
    "captures/redacted/capture-real-odoo-001/frame-0001.json"
  ]);
  const nonPngFramePath = "captures/redacted/capture-real-odoo-001/frame-0001.json";
  const audit = auditRealToolRunArtifacts(
    proof,
    (path) => existing.has(path),
    (path) => {
      if (path.endsWith("screen-input-evidence.json")) {
        return JSON.stringify({
          ...screenInputEvidence("odoo"),
          redactedFrameEvidencePaths: [nonPngFramePath]
        });
      }
      ...
      if (path.endsWith("manifest.json")) {
        return JSON.stringify({
          ...normalizedCaptureManifest("odoo"),
          redactedFrames: [{ frameId: "frame-0001", path: nonPngFramePath, visibleText: ["demo"] }]
        });
      }
...
test("real target-tool run artifact audit rejects below-threshold highlights in step trace", () => {
          ...
          steps: [
            {
              ...
              overlayKind: "fail-closed",
              overlayConfidence: 0.5,
              overlayMessage: "click the highlighted opportunity anyway.",
  ...
  assert.equal(audit.passed, false);
  assert.equal(audit.findings.some((finding) => finding.message.includes("overlayConfidence must be at least 0.75")), true);
  assert.equal(audit.findings.some((finding) => finding.message.includes("below-threshold overlayMessage must exactly match fail-closed guidance")), true);
  assert.equal(audit.findings.some((finding) => finding.message.includes("below-threshold overlay must not highlight a target")), true);
});
...
test("real target-tool run artifact audit rejects ungrounded step visible text evidence", () => {
  ...
  assert.equal(
    audit.findings.some((finding) =>
      finding.message.includes("step-001 matchedVisibleText must match flow expected visible text")
    ),
    true
  );
});

test("real target-tool run artifact audit rejects partial matched visible text evidence", () => {
  const proof = realToolProof("odoo");
  const existing = new Set([
    "evals/runs/odoo/real-proof/step-trace.json",
    "evals/runs/odoo/real-proof/final-screen.png",
