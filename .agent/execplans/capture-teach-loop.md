...
- changed: `proof:scan` now rejects bearer-token text, `secret_key` style api secrets, and `sessionid` or `session_id` leaks in shareable text artifacts, aligning the built-in scan more closely with redaction's forbidden secret classes.
- changed: CLI scan regression coverage now includes bearer token, secret key, and session id examples alongside email, password, token assignment, raw capture, local tmp, file URL, and traversal leaks.
- eval showed: the first standalone `proof:scan` run was invalid because it raced with the CLI test's temporary negative scan fixture; rerunning after the test cleanup passed. Fixture proof still passes for both tools and no real odoo/notion proof files were created.
...
- stuck point: full-goal proof is still blocked on real odoo and notion run directories with native screen-plus-input capture/eval artifacts, shareable evidence that passes the stricter bearer/api/session scan, terminal outcome evidence, and senior reviewer acceptance.
- overlay misread: none in fixture evals; no real overlay misread evidence exists yet.
- next best experiment: initialize one real target-tool run, fill same-run evidence, run `proof:scan` before `proof real-run`, and verify no shareable real-run text leaks bearer tokens, secret keys, session ids, local paths, raw capture references, emails, or other secrets.
- `pnpm --filter @onboardai/cli test`: passed; 10 tests, 10 pass, 0 fail.
- first `pnpm proof:scan`: invalid concurrent run; failed because it observed the CLI test's temporary negative scan artifact.
- second `pnpm proof:scan`: passed; scanned 28 shareable text files.
- `pnpm lint`: passed.
- `pnpm typecheck`: passed.
...
- changed: `proof:scan` now rejects local tmp paths, `file://` URLs, and traversal paths in shareable text artifacts, in addition to forbidden secrets and raw/unsafe/tmp capture references.
- changed: CLI scan regression coverage now seeds a shareable report with an email, password, token, raw capture path, local tmp path, file URL, and traversal path and requires the scan to reject every class.
- eval showed: fixture proof still passes for both tools and the hardened shareable scan passes the current committed artifacts; no real odoo/notion proof files were created.
- completion rate: deterministic fixture evals remain 6/6 taught steps; real odoo/notion held-out eval completion remains 0/2 tools because no actual real target-tool runs are present.
- stuck point: full-goal proof is still blocked on real odoo and notion run directories with native screen-plus-input capture/eval artifacts, shareable evidence that passes the stricter scan, terminal outcome evidence, and senior reviewer acceptance.
- overlay misread: none in fixture evals; no real overlay misread evidence exists yet.
- next best experiment: initialize one real target-tool run, fill same-run evidence, run `proof:scan` before `proof real-run`, and verify no shareable real-run text leaks local tmp, file URL, traversal, raw capture, email, or secret references.
- `pnpm --filter @onboardai/cli test`: passed; 10 tests, 10 pass, 0 fail.
...
- completion rate: deterministic fixture evals remain 6/6 taught steps; real odoo/notion held-out eval completion remains 0/2 tools because no actual real target-tool runs are present.
- stuck point: full-goal proof is still blocked on real odoo and notion run directories with native screen-plus-input capture/eval artifacts, explicit non-automation overlay trace evidence, terminal outcome evidence, and senior reviewer acceptance.
- overlay misread: none in fixture evals; no real overlay misread evidence exists yet.
- next best experiment: initialize one real target-tool run and verify the filled `step-trace.json` has `overlayCanAutomateInput: false`, manual-only actions, held-out frames, and post-action success observations for every taught step before dry-running `proof real-run`.
...
- changed: `docs/flow-format.md` now documents the single terminal rule and its relationship to `outcome-evidence.json` terminal visible-text matching.
...
- completion rate: deterministic fixture evals remain 6/6 taught steps; real odoo/notion held-out eval completion remains 0/2 tools because no actual real target-tool runs are present.
- stuck point: full-goal proof is still blocked on real odoo and notion run directories with native screen-plus-input capture/eval artifacts, one-terminal normalized flows from real senior demonstrations, redacted shareable evidence, terminal outcome evidence, and senior reviewer acceptance.
- overlay misread: none in fixture evals; no real overlay misread evidence exists yet.
- next best experiment: initialize one real target-tool run, generate or fill a real-demonstration `flow.md` with exactly one terminal step, then dry-run `proof real-run` before writing any summary.
- `pnpm --filter @onboardai/flow test`: passed; 9 tests, 9 pass, 0 fail.
- `pnpm flow:validate`: passed; validated 2 flow files.
- `pnpm lint`: passed.
- `pnpm typecheck`: passed.
...
- changed: normalized capture manifests now include a safe `captureId` on every sanitized raw artifact summary.
- changed: capture normalization, run-local manifest validation, and the real-run audit reject mixed raw artifact capture IDs, binding screen recording, keyboard log, mouse log, and optional human notes to one senior demonstration session without exposing raw paths.
- changed: fixture normalized manifests, same-run fixture capture manifests, the real-run capture manifest template, eval protocol, and real-run runbook now carry the same-session capture ID requirement.
- eval showed: targeted capture coverage rejects mixed capture sessions; targeted real-run audit coverage rejects copied/mixed manifest raw artifacts; CLI complete real-run dry-run coverage still accepts a complete synthetic run when artifact capture IDs match.
- completion rate: deterministic fixture evals remain 6/6 taught steps; real odoo/notion held-out eval completion remains 0/2 tools because no actual real target-tool runs are present.
- stuck point: full-goal proof is still blocked on real odoo and notion run directories with native screen-plus-input capture/eval artifacts, same-session senior capture manifests, redacted shareable evidence, terminal outcome evidence, and senior reviewer acceptance.
- overlay misread: none in fixture evals; no real overlay misread evidence exists yet.
- next best experiment: initialize one real target-tool run and ensure `capture-manifest.json` raw artifact summaries all share the manifest `captureId` before running `proof real-run`.
- `pnpm --filter @onboardai/capture test`: passed; 17 tests, 17 pass, 0 fail.
- `pnpm --filter @onboardai/eval-harness test`: passed; 63 tests, 63 pass, 0 fail.
- `pnpm --filter @onboardai/cli test`: passed; 10 tests, 10 pass, 0 fail.
- `pnpm lint`: passed.
- `pnpm typecheck`: passed.
- `pnpm test`: passed; 113 tests, 113 pass, 0 fail.
- `pnpm flow:validate`: passed; validated 2 flow files.
- `pnpm proof:fixtures`: passed; fixture proof passed 2/2 tools.
- `pnpm proof:scan`: passed; scanned 28 shareable text files.
- `pnpm proof:status`: expected failure; `full goal not proven`, `fixture proof passed`, `real-tool proof failed: 0/2 tools`.
...
- changed: run-local normalized capture manifests now declare a top-level `runId` in addition to `captureId`, `flowId`, `flowPath`, and `tool`.
- changed: run-local manifest validation and the real-run audit reject same-run `capture-manifest.json` artifacts copied from another run directory, while legacy `captures/normalized/<capture-id>/manifest.json` artifacts remain accepted without `runId`.
- changed: fixture same-run capture manifests, the real-run template, eval protocol, and real-run runbook now document the matching-run requirement.
- eval showed: targeted capture coverage rejects mismatched run ids; targeted real-run audit coverage rejects copied same-run capture manifests; CLI complete real-run dry-run coverage still accepts a complete synthetic run when `runId` matches.
- completion rate: deterministic fixture evals remain 6/6 taught steps; real odoo/notion held-out eval completion remains 0/2 tools because no actual real target-tool runs are present.
- stuck point: full-goal proof is still blocked on real odoo and notion run directories with native screen-plus-input capture/eval artifacts, run-bound same-session capture manifests, redacted shareable evidence, terminal outcome evidence, and senior reviewer acceptance.
- overlay misread: none in fixture evals; no real overlay misread evidence exists yet.
- next best experiment: initialize one real target-tool run and confirm `capture-manifest.json` declares the same `runId` as `evals/runs/<tool>/<run-id>/` before running `proof real-run`.
- `pnpm --filter @onboardai/capture test`: passed; 18 tests, 18 pass, 0 fail.
- `pnpm --filter @onboardai/eval-harness test`: passed; 64 tests, 64 pass, 0 fail.
- `pnpm --filter @onboardai/cli test`: passed; 10 tests, 10 pass, 0 fail.
- `pnpm lint`: passed.
- `pnpm typecheck`: passed.
- `pnpm test`: passed; 115 tests, 115 pass, 0 fail.
- `pnpm flow:validate`: passed; validated 2 flow files.
- `pnpm proof:fixtures`: passed; fixture proof passed 2/2 tools.
- `pnpm proof:scan`: passed; scanned 28 shareable text files.
- `pnpm proof:status`: expected failure; `full goal not proven`, `fixture proof passed`, `real-tool proof failed: 0/2 tools`.
...
- changed: real-run `screen-input-evidence.json` now declares a top-level `runId` and the real-run audit rejects screen/input evidence copied from another run directory.
- changed: the real-run screen-input template, CLI init coverage, eval protocol, and real-run runbook now require `screen-input-evidence.json` `runId` to match `evals/runs/<tool>/<run-id>/`.
- eval showed: targeted eval-harness coverage rejects copied screen-input evidence; CLI real-run init writes the run id; CLI complete-run dry-run still accepts a synthetic run when screen-input evidence matches the run directory.
- completion rate: deterministic fixture evals remain 6/6 taught steps; real odoo/notion held-out eval completion remains 0/2 tools because no actual real target-tool runs are present.
- stuck point: full-goal proof is still blocked on real odoo and notion run directories with native screen-plus-input capture/eval artifacts, run-bound screen-input evidence, run-bound capture manifests, terminal outcome evidence, and senior reviewer acceptance.
- overlay misread: none in fixture evals; no real overlay misread evidence exists yet.
- next best experiment: initialize one real target-tool run and confirm `screen-input-evidence.json`, `capture-manifest.json`, `step-trace.json`, reviewer checklist, and flow evidence all agree on the same run id before running `proof real-run`.
- `pnpm --filter @onboardai/eval-harness test`: passed; 65 tests, 65 pass, 0 fail.
- `pnpm --filter @onboardai/cli test`: passed; 10 tests, 10 pass, 0 fail.
- `pnpm lint`: passed.
- `pnpm typecheck`: passed.
- `pnpm test`: passed; 116 tests, 116 pass, 0 fail.
- `pnpm flow:validate`: passed; validated 2 flow files.
- `pnpm proof:fixtures`: passed; fixture proof passed 2/2 tools.
- `pnpm proof:scan`: passed; scanned 28 shareable text files.
- `pnpm proof:status`: expected failure; `full goal not proven`, `fixture proof passed`, `real-tool proof failed: 0/2 tools`.
...
- changed: real-run `outcome-evidence.json` now declares a top-level `runId` and the real-run audit rejects outcome evidence copied from another run directory.
- changed: the real-run outcome template, CLI init coverage, eval protocol, and real-run runbook now require `outcome-evidence.json` `runId` to match `evals/runs/<tool>/<run-id>/`.
- eval showed: targeted eval-harness coverage rejects copied outcome evidence; CLI real-run init writes the run id; CLI complete-run dry-run still accepts a synthetic run when outcome evidence matches the run directory.
- completion rate: deterministic fixture evals remain 6/6 taught steps; real odoo/notion held-out eval completion remains 0/2 tools because no actual real target-tool runs are present.
- stuck point: full-goal proof is still blocked on real odoo and notion run directories with native screen-plus-input capture/eval artifacts, run-bound outcome evidence, run-bound screen-input evidence, terminal outcome evidence, and senior reviewer acceptance.
- overlay misread: none in fixture evals; no real overlay misread evidence exists yet.
- next best experiment: initialize one real target-tool run and confirm `outcome-evidence.json`, `screen-input-evidence.json`, `capture-manifest.json`, `step-trace.json`, reviewer checklist, and flow evidence all agree on the same run id before running `proof real-run`.
- `pnpm --filter @onboardai/eval-harness test`: passed; 66 tests, 66 pass, 0 fail.
- `pnpm --filter @onboardai/cli test`: passed; 10 tests, 10 pass, 0 fail.
- `pnpm lint`: passed.
- `pnpm typecheck`: passed.
- `pnpm test`: passed; 117 tests, 117 pass, 0 fail.
- `pnpm flow:validate`: passed; validated 2 flow files.
- `pnpm proof:fixtures`: passed; fixture proof passed 2/2 tools.
- `pnpm proof:scan`: passed; scanned 28 shareable text files.
- `pnpm proof:status`: expected failure; `full goal not proven`, `fixture proof passed`, `real-tool proof failed: 0/2 tools`.
...
- changed: real-run `demo-data-evidence.json` now declares a top-level `runId` and the real-run audit rejects demo-data evidence copied from another run directory.
- changed: the real-run demo-data template, CLI init coverage, eval protocol, and real-run runbook now require `demo-data-evidence.json` `runId` to match `evals/runs/<tool>/<run-id>/`.
- eval showed: targeted eval-harness coverage rejects copied demo-data evidence; CLI real-run init writes the run id; CLI complete-run dry-run still accepts a synthetic run when demo-data evidence matches the run directory.
- completion rate: deterministic fixture evals remain 6/6 taught steps; real odoo/notion held-out eval completion remains 0/2 tools because no actual real target-tool runs are present.
- stuck point: full-goal proof is still blocked on real odoo and notion run directories with native screen-plus-input capture/eval artifacts, run-bound clean/sanitized demo-data evidence, run-bound outcome evidence, and senior reviewer acceptance.
- overlay misread: none in fixture evals; no real overlay misread evidence exists yet.
- next best experiment: initialize one real target-tool run and confirm `demo-data-evidence.json`, `outcome-evidence.json`, `screen-input-evidence.json`, `capture-manifest.json`, `step-trace.json`, reviewer checklist, and flow evidence all agree on the same run id before running `proof real-run`.
- `pnpm --filter @onboardai/eval-harness test`: passed; 67 tests, 67 pass, 0 fail.
- `pnpm --filter @onboardai/cli test`: passed; 10 tests, 10 pass, 0 fail.
- `pnpm lint`: passed.
- `pnpm typecheck`: passed.
- `pnpm test`: passed; 118 tests, 118 pass, 0 fail.
- `pnpm flow:validate`: passed; validated 2 flow files.
- `pnpm proof:fixtures`: passed; fixture proof passed 2/2 tools.
- `pnpm proof:scan`: initially failed while the full test suite's negative-test artifact `evals/reports/shareable-scan-validation.md` existed; rerun after tests completed passed with 28 shareable text files scanned.
- `pnpm proof:status`: expected failure; `full goal not proven`, `fixture proof passed`, `real-tool proof failed: 0/2 tools`.
- forbidden secret/email scan across `flows`, `evals`, `captures/normalized`, and `captures/redacted`: no matches.
- raw/unsafe/tmp path scan across shareable artifacts: no matches.
- `git check-ignore` for sample raw/unsafe/tmp capture paths: passed.
...
- changed: real-run `flow-evidence.json` now declares a top-level `runId` and the real-run audit rejects flow evidence copied from another run directory.
- changed: the real-run flow-evidence template, CLI init coverage, eval protocol, and real-run runbook now require `flow-evidence.json` `runId` to match `evals/runs/<tool>/<run-id>/`.
- eval showed: targeted eval-harness coverage rejects copied flow evidence; CLI real-run init writes the run id; CLI complete-run dry-run still accepts a synthetic run when flow evidence matches the run directory.
- completion rate: deterministic fixture evals remain 6/6 taught steps; real odoo/notion held-out eval completion remains 0/2 tools because no actual real target-tool runs are present.
- stuck point: full-goal proof is still blocked on real odoo and notion run directories with native screen-plus-input capture/eval artifacts, run-bound flow evidence, run-bound demo-data/outcome/screen-input evidence, and senior reviewer acceptance.
- overlay misread: none in fixture evals; no real overlay misread evidence exists yet.
- next best experiment: initialize one real target-tool run and confirm `flow-evidence.json`, `demo-data-evidence.json`, `outcome-evidence.json`, `screen-input-evidence.json`, `capture-manifest.json`, `step-trace.json`, and reviewer checklist all agree on the same run id before running `proof real-run`.
- `pnpm --filter @onboardai/eval-harness test`: passed; 68 tests, 68 pass, 0 fail.
- `pnpm --filter @onboardai/cli test`: passed; 10 tests, 10 pass, 0 fail.
- `pnpm lint`: passed.
- `pnpm typecheck`: passed.
...
- changed: real-run `capture-readiness.json` now declares a top-level `runId` and the real-run audit rejects capture-readiness evidence copied from another run directory.
- changed: the real-run capture-readiness template, CLI init coverage, eval protocol, and real-run runbook now require `capture-readiness.json` `runId` to match `evals/runs/<tool>/<run-id>/`.
- eval showed: targeted eval-harness coverage rejects copied capture-readiness evidence; CLI real-run init writes the run id; CLI complete-run dry-run still accepts a synthetic run when readiness evidence matches the run directory.
- completion rate: deterministic fixture evals remain 6/6 taught steps; real odoo/notion held-out eval completion remains 0/2 tools because no actual real target-tool runs are present.
- stuck point: full-goal proof is still blocked on real odoo and notion run directories with native screen-plus-input capture/eval artifacts, run-bound capture-readiness evidence, and senior reviewer acceptance.
- overlay misread: none in fixture evals; no real overlay misread evidence exists yet.
- next best experiment: initialize one real target-tool run and confirm `capture-readiness.json`, `screen-input-evidence.json`, `flow-evidence.json`, `demo-data-evidence.json`, `outcome-evidence.json`, `capture-manifest.json`, `step-trace.json`, and reviewer checklist all agree on the same run id before running `proof real-run`.
- `pnpm --filter @onboardai/eval-harness test`: passed; 69 tests, 69 pass, 0 fail.
- `pnpm --filter @onboardai/cli test`: passed; 10 tests, 10 pass, 0 fail.
- `pnpm lint`: passed.
- `pnpm typecheck`: passed.
- `pnpm test`: passed; 120 tests, 120 pass, 0 fail.
- `pnpm flow:validate`: passed; validated 2 flow files.
- `pnpm proof:fixtures`: passed; fixture proof passed 2/2 tools.
- `pnpm proof:scan`: passed; scanned 28 shareable text files.
- `pnpm proof:status`: expected failure; `full goal not proven`, `fixture proof passed`, `real-tool proof failed: 0/2 tools`.
...
- changed: real-run `failure-log.md` must now identify the audited tool, run id, and `flow-evidence.json` flow id before it can support a passing run.
- changed: the real-run audit rejects failure logs copied from another tool, flow, or run directory; CLI real-run init now fills `- flow id:` in markdown templates.
- changed: eval protocol and real-run runbook now require `failure-log.md` identity lines to match the audited run and flow evidence.
- eval showed: targeted eval-harness coverage rejects copied failure logs; CLI real-run init writes the flow id; CLI complete-run dry-run still accepts a synthetic run when the failure log identity matches the run directory.
- completion rate: deterministic fixture evals remain 6/6 taught steps; real odoo/notion held-out eval completion remains 0/2 tools because no actual real target-tool runs are present.
- stuck point: full-goal proof is still blocked on real odoo and notion run directories with native screen-plus-input capture/eval artifacts, run-bound failure logs, and senior reviewer acceptance.
- overlay misread: none in fixture evals; no real overlay misread evidence exists yet.
- next best experiment: initialize one real target-tool run and confirm every markdown and JSON evidence artifact agrees on tool, run id, and flow id before running `proof real-run`.
- `pnpm --filter @onboardai/eval-harness test`: passed; 70 tests, 70 pass, 0 fail.
- `pnpm --filter @onboardai/cli test`: passed; 10 tests, 10 pass, 0 fail.
- `pnpm lint`: passed.
- `pnpm typecheck`: passed.
- `pnpm test`: passed; 121 tests, 121 pass, 0 fail.
- `pnpm flow:validate`: passed; validated 2 flow files.
- `pnpm proof:fixtures`: passed; fixture proof passed 2/2 tools.
- `pnpm proof:scan`: passed; scanned 28 shareable text files.
- `pnpm proof:status`: expected failure; `full goal not proven`, `fixture proof passed`, `real-tool proof failed: 0/2 tools`.
...
- changed: `proof real-run init <tool> --generate-run-id` now creates a safe unused run id shaped like `<tool>-real-eval-<utc-timestamp>` and initializes the same editable real-run skeleton.
- changed: generated-id init still creates no live `real-tool-proof-*.json` summary and the generated skeleton still fails `proof real-run` until real held-out screen-plus-input evidence, reviewer acceptance, and outcome evidence are filled.
- changed: eval protocol and real-run runbook now document generated run ids for operators who do not already have a stable run id.
- eval showed: targeted CLI coverage proves generated run ids are lowercase kebab-case, tool-prefixed, substituted through run artifacts, and rejected by `proof real-run` while evidence is incomplete.
- completion rate: deterministic fixture evals remain 6/6 taught steps; real odoo/notion held-out eval completion remains 0/2 tools because no actual real target-tool runs are present.
- stuck point: full-goal proof is still blocked on real odoo and notion run directories with native screen-plus-input capture/eval artifacts, accepted reviewer checklists, and live real-proof summaries written only after dry-run validation.
- overlay misread: none in fixture evals; no real overlay misread evidence exists yet.
- next best experiment: run `proof real-run init <tool> --generate-run-id` for one real target-tool eval session, fill the generated directory with actual clean-demo native capture/eval artifacts, then dry-run `proof real-run` before writing any summary.
- `pnpm --filter @onboardai/cli test`: passed; 11 tests, 11 pass, 0 fail.
- `pnpm lint`: passed.
- `pnpm typecheck`: passed.
- `pnpm test`: passed; 123 tests, 123 pass, 0 fail.
- `pnpm flow:validate`: passed; validated 2 flow files.
- `pnpm proof:fixtures`: passed; fixture proof passed 2/2 tools.
- `pnpm proof:scan`: passed; scanned 28 shareable text files.
- `pnpm proof:status`: expected failure; `full goal not proven`, `fixture proof passed`, `real-tool proof failed: 0/2 tools`.
...
- changed: same-run `evals/runs/<tool>/<run-id>/capture-manifest.json` artifacts must now list every redacted `source-frame` referenced by the taught `flow.md` screen-region hints.
- changed: the real-run audit rejects same-run capture manifests that name the right flow id/path but omit the actual flow source frames, preventing a manifest from being paired with unrelated visual evidence.
- changed: CLI complete-run fixture coverage and docs now include the required flow source frames in same-run normalized capture manifests.
- eval showed: targeted eval-harness coverage rejects a same-run capture manifest missing `captures/redacted/odoo-qualify-opportunity/frame-0001.png`; same-run manifest acceptance still passes when all taught source frames are listed.
- completion rate: deterministic fixture evals remain 6/6 taught steps; real odoo/notion held-out eval completion remains 0/2 tools because no actual real target-tool runs are present.
- stuck point: full-goal proof is still blocked on real odoo and notion run directories with native screen-plus-input capture/eval artifacts whose same-run manifests include the taught flow source frames.
- overlay misread: none in fixture evals; no real overlay misread evidence exists yet.
- next best experiment: run `proof real-run init <tool> --generate-run-id`, normalize a real senior demonstration so the same-run manifest includes every `flow.md` source frame, then dry-run `proof real-run` before writing any summary.
- `pnpm --filter @onboardai/eval-harness test`: passed; 72 tests, 72 pass, 0 fail.
- `pnpm --filter @onboardai/cli test`: passed; 11 tests, 11 pass, 0 fail.
- `pnpm lint`: passed.
- `pnpm typecheck`: passed.
- `pnpm test`: passed; 124 tests, 124 pass, 0 fail.
- `pnpm flow:validate`: passed; validated 2 flow files.
- `pnpm proof:fixtures`: passed; fixture proof passed 2/2 tools.
- `pnpm proof:scan`: passed; scanned 28 shareable text files.
- `pnpm proof:status`: expected failure; `full goal not proven`, `fixture proof passed`, `real-tool proof failed: 0/2 tools`.
...
- changed: same-run `capture-manifest.json` source frames must now include the expected visible text for the corresponding taught `flow.md` step.
- changed: the real-run audit rejects a manifest that lists the correct `source-frame` path but whose `visibleText` omits required screen text, preventing path-only visual grounding.
- changed: eval protocol and real-run runbook now require same-run source frame `visibleText` to include the flow step expected visible text.
- eval showed: targeted eval-harness coverage rejects `captures/redacted/odoo-qualify-opportunity/frame-0001.png` when its manifest `visibleText` omits `new`; same-run manifest acceptance still passes when all source-frame visible text matches expected flow text.
- completion rate: deterministic fixture evals remain 6/6 taught steps; real odoo/notion held-out eval completion remains 0/2 tools because no actual real target-tool runs are present.
- stuck point: full-goal proof is still blocked on real odoo and notion run directories with native screen-plus-input capture/eval artifacts whose same-run manifests include source frames and visible text for every taught step.
- overlay misread: none in fixture evals; no real overlay misread evidence exists yet.
- next best experiment: run `proof real-run init <tool> --generate-run-id`, normalize a real senior demonstration into a same-run manifest whose source-frame `visibleText` proves every step's expected state, then dry-run `proof real-run`.
- `pnpm --filter @onboardai/eval-harness test`: passed; 73 tests, 73 pass, 0 fail.
- `pnpm --filter @onboardai/cli test`: passed; 11 tests, 11 pass, 0 fail.
- `pnpm lint`: passed.
- `pnpm typecheck`: passed.
- `pnpm test`: passed; 125 tests, 125 pass, 0 fail.
- `pnpm flow:validate`: passed; validated 2 flow files.
- `pnpm proof:fixtures`: passed; fixture proof passed 2/2 tools.
- `pnpm proof:scan`: passed; scanned 28 shareable text files.
- `pnpm proof:status`: expected failure; `full goal not proven`, `fixture proof passed`, `real-tool proof failed: 0/2 tools`.
